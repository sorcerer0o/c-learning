...
## 自测
  ...
  1. C 语言中 sizeof 是函数还是运算符？返回值类型是什么？
  2. short、int、long、long long 的大小关系是怎样的？
  3. 浮点数为什么不能用 == 直接比较？应该怎么比较？
  4. 原码、反码、补码分别是什么？计算机中存储的是哪种？
  5. 位运算符 &、|、^、~、<<、>> 各自的作用是什么？
  6. 如何用位运算判断一个数是奇数还是偶数？
  7. 如何用异或交换两个变量？这个写法有什么限制？
  8. 隐式类型转换中，无符号和有符号混用会有什么陷阱？
  9. 如何用位运算设置/清除/翻转某个二进制位？
  10. getchar 的返回值为什么是 int 而不是 char？
