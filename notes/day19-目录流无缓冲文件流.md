...
1. 目录流的使用步骤是什么？opendir/readdir/closedir 各起什么作用？
2. readdir 返回的 dirent 结构体有哪些关键字段？
3. dirent 的 d_name 缓冲区大小是多少？是否需要 free？
4. stat 函数获取文件信息的步骤是什么？如何判断文件类型？
5. 有缓冲 IO 和无缓冲 IO 的区别是什么？各自适用什么场景？
6. 文件描述符 0/1/2 分别代表什么？新打开的文件描述符从几开始？
7. open 函数的 flags 参数有哪些常用组合？O_CREAT 和 O_TRUNC 的作用是什么？
8. read 的返回值分别代表什么含义（>0、=0、-1）？
9. mmap 的原理是什么？相比 read/write 有什么优势？
10. dup2(oldfd, newfd) 的作用是什么？如何用它将 stdout 重定向到文件？
